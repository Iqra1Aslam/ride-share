...
//         const profile_image_url = await upload_on_cloudinary(image);
//         try {
...
export const driver = {
    driver_details_add: asyncHandler(async (req, res) => {
        const user_id = req.user_id;
        // let user_id = req.params.id.trim();
        const { name, phone, cnic } = req.body;
    ...
    upload_driver_license_image: asyncHandler(async (req, res) => {
        try {
            ...
            // Correctly get the URL from Cloudinary
            const profile_image_url = await upload_on_cloudinary(image);
        const driver = await Driver.findByIdAndUpdate(
            user_id,
            { driver_lisence_image: profile_image_url },
        ...
        return res.status(201).json(new ApiResponse(201, { driver }, 'Successfully update image'))
